...
import { UserService as IUserService, ResourcePayload, UserPayload } from '@dragic/auth';
import { Cache } from '@dragic/cache';
import { DrizzleService, UserRow, users } from '@dragic/database';
...
export class UserService implements IUserService<UserEntity> {
  ...
  async findOrCreateByOAuth(profile: any): Promise<UserEntity> {
    // TODO: 实现 OAuth 用户查找或创建逻辑
    throw new Error('Method not implemented - use auth-server for OAuth');
  }

  async findOrCreateByPhone(phone: string): Promise<UserEntity> {
    // TODO: 实现手机号用户查找或创建逻辑
    throw new Error('Method not implemented - use auth-server for phone auth');
  }
}
